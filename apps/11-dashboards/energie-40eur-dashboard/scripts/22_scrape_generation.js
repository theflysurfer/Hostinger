/**
 * Script 22: Generation by Type Scraper for ENTSO-E
 * Scrapes actual electricity generation by production type (nuclear, solar, wind, etc.)
 * Usage: node scripts/22_scrape_generation.js 2024
 */
...
async function main() {
    ...
    const outputFile = path.join(__dirname, '..', 'data', 'raw', `entsoe_generation_${year}_scraped.jsonl`);
    ...
    for (let i = 0; i < datesToScrape.length; i++) {
        ...
        try {
            // URL format for Actual Generation per Type (NEW PLATFORM)
            // Area: France (10YFR-RTE------C)
            // Date format must be array: ["YYYY-MM-DD", "YYYY-MM-DD"]
            const url = `https://newtransparency.entsoe.eu/generation/actual/perType/generation?appState=%7B%22sa%22%3A%5B%22CTY%7C10YFR-RTE------C%22%5D%2C%22st%22%3A%22CTY%22%2C%22mm%22%3Afalse%2C%22ma%22%3Afalse%2C%22dt%22%3A%22TABLE%22%2C%22df%22%3A%5B%22${dateStr}%22%2C%22${dateStr}%22%5D%2C%22tz%22%3A%22CET%22%7D`;
            ...
            const data = await page.evaluate((date) => {
                ...
                // Order matches screenshot from 2024-06-15
                const headerNames = [
                    'Biomass',
                    'Energy storage',
