...
- **Status checks** désactivés (voir note ci-dessous)

> **⚠️ Note importante:** Les status checks ont été **complètement désactivés** pour éviter les conflits avec le système d'auto-stop Docker. Les requêtes de health check généraient du trafic toutes les minutes, empêchant les services de s'arrêter automatiquement.
...
> **⚠️ STATUS CHECKS DÉSACTIVÉS** (depuis 24/10/2025)

Les status checks ont été **complètement supprimés** de la configuration pour résoudre un conflit avec le système d'auto-stop Docker.

### Problème Résolu

**Symptôme:** Les requêtes de health check (axios/1.12.0) étaient envoyées toutes les 60 secondes, empêchant les services de s'arrêter automatiquement après 15 minutes d'inactivité.

**Solution:** Suppression complète des lignes `statusCheck` du fichier de configuration. **Important:** Mettre `statusCheck: false` ne suffit pas à cause du cache localStorage des navigateurs.
...
### Si Vous Avez Besoin de Status Checks

Si vous devez réactiver les status checks malgré le conflit avec auto-stop :

1. Ajouter dans `appConfig`:
```yaml
statusCheck: true
...
### Status Checks Causing Auto-Stop Issues

**Symptôme:** Services ne s'arrêtent jamais automatiquement (auto-stop ne fonctionne pas)

**Cause:** Les requêtes de status check réinitialisent le compteur d'inactivité

**Solution:** Supprimer complètement les lignes `statusCheck` et `statusCheckInterval` de `conf.yml`
